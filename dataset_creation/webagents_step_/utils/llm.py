...
def parse_action_reason(model_response):
    ...
    action_description_match = re.search(r'ACTION DESCRIPTION:\s*(.*?)\s*(?=\n[A-Z]|$)', model_response, re.DOTALL)
...
def construct_llm_message_openai(prompt, prompt_mode):
    ...
    if prompt["examples"]:
        # print(f"I am at llm.py EXAMPLE is present")
        messages.append({"role": "system", "content": "Here are a few examples:"})
        for example in prompt["examples"]:
            messages.append({"role": "system", "content": f"\n### Input:\n{example['input']}\n\n### Response:\n{example['response']}"})
    
    messages.append({"role": "user", "content": f"Here is the current Input. Please respond with REASON, ACTION and ACTION DESCRIPTION.\n### Input:\n{prompt['input']}\n\n### Response:"})
    if prompt_mode == "chat":
        return messages
    elif prompt_mode == "completion":
        all_content = ''.join(message['content'] for message in messages)
...
import sys
import os
sys.path.append(os.path.abspath("/Users/aatmanj/Desktop/SteP/webagents-step"))

from testee import caller
    ...
    while True:
        try:
            ...
            for mess in messages:
                final_message+=mess['content']
            answer = caller(final_message)
            return answer
